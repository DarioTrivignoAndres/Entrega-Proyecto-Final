...
namespace WebApplication2
{
    ...
    public static class ADO_ProductoVendido
    {
        ...
        public static List<ProductoVendido> TraerProductosVendidos_conIdUsuario(long idUsuario)
        {
            ...
            Usuario usuarioEnBD = ADO_Usuario.TraerUsuario_conId(idUsuario); 
            if (usuarioEnBD.Id == 0) 
            {
            ...
            {
                const string query = "SELECT pv.Id, pv.Stock, pv.IdProducto, pv.IdVenta " + "FROM[SistemaGestion].[dbo].[ProductoVendido] " +
                    "AS pv " + "INNER JOIN Producto p ON p.Id = pv.IdProducto " + "WHERE p.IdUsuario = @idUsuario";                       
        ...
        public static List<ProductoVendido> TraerProductosVendidos_conIdVenta(long idVenta) 
        {
            ...
            using (SqlConnection sqlConnection = new SqlConnection(connectionString))
            {
                const string query =    "SELECT * FROM [SistemaGestion].[dbo].[ProductoVendido] WHERE IdVenta = @idVenta";
        ...
        public static bool EliminarProductoVendido(long idProducto)
        {
            ...
            using (SqlConnection sqlConnection = new SqlConnection(connectionString))
            {
                string queryUpdate = "DELETE FROM [SistemaGestion].[dbo].[ProductoVendido] " + "WHERE IdProducto = @idProducto";
