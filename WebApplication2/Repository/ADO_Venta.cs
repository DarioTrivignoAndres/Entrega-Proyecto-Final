...
namespace WebApplication2
{
    ...
    public static class ADO_Venta
    {
        ...
        public static List<Venta> TraerVentas_conIdUsuario(long idUsuario)
        {
            ...
            {
                const string query =    "SELECT v.Id, v.Comentarios " + "FROM Venta AS v " + "INNER JOIN ProductoVendido AS pv " + "ON v.Id = pv.IdVenta " +
                                        "INNER JOIN Producto AS p " + "ON pv.IdProducto = p.Id " + "WHERE p.IdUsuario = @idUsuario ";
                    ...
                    using (SqlDataReader dataReader = sqlCommand.ExecuteReader())
                    {
                        if (dataReader.HasRows)
                        {
                            while (dataReader.Read())
                            {
                                ...
                                venta.Id = Convert.ToInt32("Id");
                                venta.Comentarios = Convert.ToString("Comentarios");
        ...
        public static List<ProductoVendido> TraerVentas()
        {
            ...
            using (SqlConnection sqlConnection = new SqlConnection(connectionString))
            {
                const string query = "SELECT pv.Id, pv.Stock, pv.IdProducto, pv.IdVenta " + "FROM Venta AS v " + 
        ...
        public static void MostrarVentas(List<Venta> ventas)
        {
            ...
            {
                Console.WriteLine("id: " + item.Id.ToString() + "\tcomentarios: " + item.Comentarios.ToString());
            }
            Console.WriteLine(" ");
        }
    }
